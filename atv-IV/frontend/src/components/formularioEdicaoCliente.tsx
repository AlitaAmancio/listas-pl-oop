...
export default function FormularioEdicaoCliente(props: Props) {
    ...
    const [cliente, setCliente] = useState(location.state || clienteObj);
    ...
    function validar() {
        if (!cliente || cliente.nome.trim().length === 0) return false;
        if (cliente.nomeSocial.trim().length === 0) return false;
        if (!cliente.email && cliente.email !== "") return false;
        ...
        if (!cliente.telefones || cliente.telefones.length === 0) return false;

        for (const tel of cliente.telefones) {
            if (tel.ddd && tel.ddd.trim().length === 0) return false;
            if (tel.numero && tel.numero.trim().length === 0) return false;
        }
    ...
    return (
        <div className="container-fluid">
            <div className="margin-lista">
                ...
                <h3 style={{ textAlign: "center" }}>Informações do cliente:</h3>
                ...
                <div className="m-4">
                    {cliente.telefones.map((tel: { ddd: string; numero: string }, i: number) => (
